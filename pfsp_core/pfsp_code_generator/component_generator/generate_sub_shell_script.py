...
class GenerateSubShellScript():
    ...
    def __write_default_initial_code(self):
        with open(os.path.join(self.__shell_script_dirname, self.__shell_script_file_name), 'a+') as f:
            ...
            f.write('cd $current_Path/pfsp_core/$build_path\n')
            f.write('make\n\n')

            f.write('cd $current_Path/pfsp_core/$build_path/pfsp_component_plugin/error_handler\n')
            f.write('cp -rf libPFSP_plugin_error_handler.so $current_Path/pfsp_core/pfsp_lib/libs\n\n')

            f.write('cd $current_Path/pfsp_core/$build_path/pfsp_component_plugin/mode_manager\n')
            f.write('cp -rf libPFSP_plugin_mode_manager.so $current_Path/pfsp_core/pfsp_lib/libs\n\n')
            f.close()

    def __write_default_ending_code(self):
        with open(os.path.join(self.__shell_script_dirname, self.__shell_script_file_name), 'a+') as f:
            f.write('export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$current_Path/pfsp_core/pfsp_lib/libs\n')
            f.write('cd $current_Path/pfsp_core/$build_path/pfsp_main\n')
            f.write('sudo ./PFSP_main\n')
            f.close()

    def __write_each_component_lib(self):
        with open(os.path.join(self.__shell_script_dirname, self.__shell_script_file_name), 'a+') as f:
            f.write(f'cd $current_Path/pfsp_core/$build_path/pfsp_component_plugin/component_generated/base_host_component_{self.__component_id}\n')
            f.write(f'cp -rf libPFSP_base_host_component_{self.__component_id}.so $current_Path/pfsp_core/pfsp_lib/libs\n\n')

            f.write(f'cd $current_Path/pfsp_core/$build_path/$build_path/{self.__component_name}\n')
            f.write(f'cp -rf libPFSP_{self.__component_name}.so $current_Path/pfsp_core/pfsp_lib/libs\n\n')
            f.close()