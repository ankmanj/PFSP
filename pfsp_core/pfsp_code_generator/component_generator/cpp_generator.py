...
class CppFunction(CppLanguageElement):
    ...
    availablePropertiesNames = {'ret_type',
                                ...
                                'is_method',
                                'is_template',
                                'ext_obj'} | CppLanguageElement.availablePropertiesNames
    ...
    def __init__(self, **properties):
        ...
        self.internal_template_elements = None
    ...
    def add_template(self, cpp_template):
        '''
        @param: add_template CppTemplate instance
        '''
        cpp_template.ref_to_parent = self    
        self.internal_template_elements = cpp_template
    ...
    def implementation(self, cpp):
        ...
        if self.implementation_handle is not None:
            if self.ext_obj is None:
                ...
                self.implementation_handle(self, cpp, self.ext_obj)
    ...
    def render_to_string_declaration(self, cpp):
        ...
        if self.is_template:
            self.internal_template_elements.render_to_string_declaration(cpp)
            #without semicolon for template definition inside class
            template_func_signature = '{0}{1} {2}({3}){4}{5}'.format('virtual ' if self.is_virtual else '',
                                            self.ret_type if self.ret_type else '',
                                            self.name,
                                            ', '.join(self.arguments),
                                            ' const ' if self.is_const else '',
            ...
            with cpp.block(template_func_signature): 
...
class CppClass(CppLanguageElement):    
    ...
    availablePropertiesNames = {'is_struct',
                                'documentation',
                                'parent_class',
                                'is_template'} | CppLanguageElement.availablePropertiesNames
    ...
    def __init__(self, **properties):
        ...
        # class templates
        self.internal_template_elements = []

        # class includes
        self.internal_includes = None

        #Using defnitions
        self.using_element = None

        #Class declaration specific lines 
        self.decl_specific_lines = None 

        #Class Impl specific lines 
        self.impl_specific_lines = None
    ...
    def add_template(self, cpp_template):
        ...
        '''
        cpp_template.ref_to_parent = self
        #cpp_template.is_class_member = True
        #self.internal_template_elements.append(cpp_template)
        self.internal_template_elements = cpp_template    
    ...
    def add_includes(self, cpp_include):
        '''
        @param: add_includes CppTemplate instance
        '''
        cpp_include.ref_to_parent = self
        self.internal_includes = cpp_include

    def add_using(self, cpp_using):
        '''
        @param: add_includes CppTemplate instance
        '''
        cpp_using.ref_to_parent = self
        self.using_element = cpp_using
    ...
    def add_header_specific_lines(self, input_str):
        '''
        @param: input_str containing header specific lines 
        '''
        #enum.ref_to_parent = self
        self.decl_specific_lines = input_str

    def add_impl_specific_lines(self, input_str):
        '''
        @param: input_str containing header specific lines 
        '''
        #enum.ref_to_parent = self
        self.impl_specific_lines = input_str
    ...
    def render_to_string_declaration(self, cpp):
        ...
        '''
        if self.internal_includes is not None:
            self.internal_includes.render_to_string_declaration(cpp)

        if self.using_element is not None:
            self.using_element.render_to_string_declaration(cpp)
        ...
        with cpp.block('{0} {1} {2}'.format(class_type,
            ...
            #Render header specific lines here
            if self.decl_specific_lines is not None: 
                cpp(self.decl_specific_lines)
                cpp.newline()
            ...
            # in case of struct all members meant to be public
            if not self.is_struct and len(self.internal_variable_elements) != 0:
                cpp.label('private')
            self.private_class_members(cpp)

    def render_to_string_implementation(self, cpp, cpp_header_file = None):
        '''
        Render to string class definition.
        Typically handle to *.cpp file should be passed as 'cpp' param
        '''
        if cpp_header_file is not None:
            cpp('#include "{0}"'.format(cpp_header_file.filename))
        cpp.newline(2)

        #Add implementation specific lines
        if self.impl_specific_lines is not None:
            cpp(self.impl_specific_lines)
            cpp.newline(1)
        ##
...
class CppTemplateClass(CppLanguageElement):
    def __init__(self, **properties):        
        # check properties
        input_property_names = set(properties.keys())
        self.check_input_properties_names(input_property_names)
        super(CppTemplateClass, self).__init__(properties)

        self.init_class_properties(current_class_properties=self.availablePropertiesNames,
                                   input_properties_dict=properties)       

        # agregated classes
        self.template_params_elements = []

    ########################################
    # ADD template parameters
    def add_template_params(self, template_param):
        '''
        @param: enum CppTemplateClass instance
        '''
        #template_param.ref_to_parent = self+
        if template_param not in self.template_params_elements:
            self.template_params_elements.append(template_param)
    ...
    def render_to_string_declaration(self, cpp):
        ...
        cpp('{0}{1} {2}'.format('template<class ',                                            
                                            ' , class '.join(self.template_params_elements),
                                            '>'))
        
class CppTemplateFunction(CppLanguageElement):
    def __init__(self, **properties):        
        # check properties
        input_property_names = set(properties.keys())
        self.check_input_properties_names(input_property_names)
        super(CppTemplateFunction, self).__init__(properties)
...
class CppInclude(CppLanguageElement):
    def __init__(self, **properties):
        # check properties
        input_property_names = set(properties.keys())
        self.check_input_properties_names(input_property_names)
        super(CppInclude, self).__init__(properties)
    ...
    def add_includes(self, include_file_name):
        '''
        @param: include file name.
        '''
        if include_file_name not in self.includes_list:
            self.includes_list.append(include_file_name)

    def render_to_string_declaration(self, cpp):
        '''
        Render to string class includes.
        Typically handle to header should be passed as 'cpp' param
        '''

        for item in self.includes_list:
            cpp('#include "{0}"'.format(item))

        cpp.newline(2)

class CppUsingDefinitions(CppLanguageElement):
    def __init__(self, **properties):
        # check properties
        input_property_names = set(properties.keys())
        self.check_input_properties_names(input_property_names)
        super(CppUsingDefinitions, self).__init__(properties)
    ...
    def add_using(self, using_elem):
        '''
        @param: include file name.
        '''
        if using_elem not in self.using_list:
            self.using_list.append(using_elem)
