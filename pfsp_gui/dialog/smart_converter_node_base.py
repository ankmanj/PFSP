...
class SmartConverterGraphicsNode(QDMGraphicsNode):
    def initSizes(self):
        super().initSizes()
        self.width = 200
        self.height = 200
...
class SmartConverterContent(QDMNodeContentWidget):
    def initUI(self):
        lbl = QLabel(self.node.content_label, self)
        lbl.setObjectName(self.node.content_label_objname)

class SmartConverterNode(Node):
    icon = ''
    op_code = 0
    op_title = 'Undefined'
    content_label = ''
    ...
    def initInnerClasses(self):
        self.content = SmartConverterContent(self)
        self.grNode = SmartConverterGraphicsNode(self)
