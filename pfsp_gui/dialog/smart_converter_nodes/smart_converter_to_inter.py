...
class SmartConverterToInterContent(QDMNodeContentWidget):
    ...
    def initUI(self):
        ...
        # self.classPostfixEdit = QLineEdit('the rest from template_out', self)
        # self.classPostfixEdit.setEnabled(False)
        hbox = QHBoxLayout()
        hbox.addWidget(QLabel('Class : ',self))
        hbox.addWidget(self.classPrefixEdit)
        # hbox.addWidget(QLabel('+', self))
        # hbox.addWidget(self.classPostfixEdit)
        self.vboxLayout.addLayout(hbox)

        # hbox = QHBoxLayout()
        # hbox.addWidget(QLabel('<template_in>'))
        # hbox.addStretch(1)
        # hbox.addWidget(QLabel('---->'))
        # hbox.addStretch(1)
        # hbox.addWidget(QLabel('<template_out>'))
        # self.vboxLayout.addLayout(hbox)

        self.functionEdit = QLineEdit("getConvertedInternalData", self)
...
class SmartConverterToInterGraphicsNode(QDMGraphicsNode):
    def initSizes(self):
        super().initSizes()
...
@register_node(SMART_CONVERTER_NODE_TO_INTER)
class PFSPNodeSmartConverterToInter(SmartConverterNode):
    icon = ''
    op_code = SMART_CONVERTER_NODE_TO_INTER
    op_title = 'SmartConverter - External Input To Internal'
    content_label_objname = 'pfsp_node_smart_converter_to_internal'
    class_name = ''
    ...
    def initInnerClasses(self):
        self.content = SmartConverterToInterContent(self)
        self.grNode = SmartConverterToInterGraphicsNode(self)
